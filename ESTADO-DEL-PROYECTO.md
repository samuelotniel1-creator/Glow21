...
4. **No existe ningún sistema de envío de correo en el proyecto** (ni Resend, SendGrid, ni nada) — el copy de `/preregistro` en algún momento llegó a prometer "te enviaremos un recordatorio" / "revisa tu correo", lo cual era falso y se corrigió (2026-08-29). Si en algún momento se quiere un correo de confirmación/recordatorio real al registrarse, hay que construirlo desde cero: elegir proveedor (Resend es el más simple), conseguir API key, y agregar el envío (probablemente desde una función serverless de Vercel, ya que el insert a Supabase lo hace el navegador directamente). No asumir que existe solo porque el copy lo sugiera en algún lugar — verificar primero.
