...
- Botón "Acceso Premium 3 meses" (lo prendes/apagas desde el admin) que abre un formulario aparte (nombre, correo, teléfono, **dirección** — porque esa oferta envía un producto físico) antes de mandar a pagar.
- Todo el contenido y la identidad visual siguen la guía de marca (`brand-glow21.skill`).

## Panel admin (`/admin`)

Configura: Zoom Meeting ID, password de Zoom, fecha/hora de la sesión (selector nativo), toggle "en vivo ahora", toggle "Acceso Premium". Ahora pide login (antes no tenía ninguna protección).
...
4. **Conectar el link de pago real** al botón de Acceso Premium (hoy manda a `/pago`, que no existe todavía).
